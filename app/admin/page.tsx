...
export default function AdminOverview() {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR("https://api.samaabysiblings.com/backend/api/v1/admin", fetcher, {
          ...
          {stats.ordersByStatus && stats.ordersByStatus.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie
                    data={((stats.ordersByStatus ?? []) as StatusCount[]).map(
                      (item: StatusCount) => ({
                        name:
                    ...
                    dataKey="value"
                  >
                    {((stats.ordersByStatus ?? []) as StatusCount[]).map(
                      (entry: StatusCount, index: number) => (
                        <Cell
                          key={`cell-${index}`}
          ...
          {stats.ordersByPayment && stats.ordersByPayment.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie
                    data={((stats.ordersByPayment ?? []) as PaymentCount[]).map(
                      (item: PaymentCount) => ({
                    ...
                    dataKey="value"
                  >
                    {((stats.ordersByPayment ?? []) as PaymentCount[]).map(
                      (entry: PaymentCount, index: number) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={COLORS[index % COLORS.length]}
                        />
          ...
          {stats.stockStatus && stats.stockStatus.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie
                    data={((stats.stockStatus ?? []) as StockStatus[]).map(
                      (item: StockStatus) => ({
                        name: item.status,
                        value: Number(item.count),
                      })
                    ...
                    dataKey="value"
                  >
                    {((stats.stockStatus ?? []) as StockStatus[]).map(
                      (entry: StockStatus) => (
                        <Cell
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {stats.topSellingItems && stats.topSellingItems.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart
                  data={(stats.topSellingItems ?? []) as any[]}
                  layout="vertical"
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                  <XAxis
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {((stats.recentOrders ?? []) as OrderSummary[])
                    .slice(0, 10)
        ...
        {stats.recentSubscribers && stats.recentSubscribers.length > 0 && (
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {((stats.recentSubscribers ?? []) as Subscriber[]).map(
                (sub: Subscriber, idx: number) => (
                  <div
                    key={idx}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
              ...
              <div className="space-y-3">
                {((stats.bundleBreakdown ?? []) as BundleBreakdown[]).map(
                  (item: BundleBreakdown, idx: number) => (
                    <div
                      key={idx}
            ...
            {stats.storiesStatus && stats.storiesStatus.length > 0 && (
              <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
                ...
                <div className="space-y-3">
                  {((stats.storiesStatus ?? []) as StoryStatus[]).map(
                    (item: StoryStatus, idx: number) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-4 bg-slate-50 rounded-lg"
                      >
